...
入力:
  ...
  {GDRIVE}/katori_features_v2_2023.csv
  ~/Downloads/香取市_labeling_150_labels.csv

出力:
  ~/Downloads/香取市_abandoned_candidates_clustered_<N>.kml
  ~/Downloads/香取市_abandoned_candidates_clustered_<N>_labels.csv

使い方:
  python scripts/create_abandoned_candidate_kml_katori.py        # 200件
  python scripts/create_abandoned_candidate_kml_katori.py 100    # 100件
"""
...
SHP_PATH     = Path.home() / "Downloads/2023_katori/2023_katori.shp"
LABELED_CSV  = Path.home() / "Downloads/香取市_labeling_150_labels.csv"
RANDOM_STATE = 42
...
V2_CSV = f"{_GDRIVE}/katori_features_v2_2023.csv"
...
def main(n_total: int = 200):
    print("=" * 60)
    print("  香取市 放棄地候補 空間クラスタリングKML生成")
    print("=" * 60)
    ...
    # ---- 森林・建物近接フィルタ（VH_mean < -16 dB） ----
    # 森林・都市建物はSAR後方散乱が強く VH_mean が高い傾向
    # 放棄地の中央値 ≈ -19 dB、除外（森林等）の中央値 ≈ -16 dB
    before = len(df)
    df = df[df["VH_mean"] < -16].copy()
    print(f"  VH_meanフィルタ後（< -16 dB）: {before - len(df)}件除去 → 残り{len(df):,}件")

    # ---- 既ラベル済みを除外 ----
    if LABELED_CSV.exists():
        labeled = pd.read_csv(LABELED_CSV, encoding="utf-8-sig")
        labeled_uu = set(labeled["polygon_uu"].dropna().astype(str))
        before = len(df)
    ...
    # ---- KML生成 ----
    n = len(selected)
    kml_path = OUTPUT_DIR / f"香取市_abandoned_candidates_clustered_{n}.kml"
    csv_path = OUTPUT_DIR / f"香取市_abandoned_candidates_clustered_{n}_labels.csv"

    kml = ET.Element("kml", xmlns="http://www.opengis.net/kml/2.2")
    doc = ET.SubElement(kml, "Document")
    ET.SubElement(doc, "name").text = "香取市 放棄地候補（空間クラスタリング）"
